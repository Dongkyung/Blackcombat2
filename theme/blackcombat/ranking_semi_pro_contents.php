...
<?
    $sql = "SELECT 
        ...
        WHERE division='플라이급' AND ranking_type = 2
        ORDER BY 
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">플라이급</span> <span class="champ">1위</span></h3>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='밴텀급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">밴텀급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='페더급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">페더급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='라이트급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">라이트급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='웰터급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">웰터급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='미들급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">미들급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='중량급' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">중량급</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
...
<?
    $sql = "SELECT 
        ...
        LEFT JOIN blackcombat.tb_team_base team
            ON base.team_seq = team.team_seq 
        WHERE division='여성부' AND ranking_type = 2
        ORDER BY 
        CASE WHEN ranking = 'c' THEN 0  
        ELSE CAST(ranking AS UNSIGNED) END, ranking;";
        $result = sql_query($sql);
...
<?              }else{ ?>
                        <div class="ranking_list_part_champ" onclick="location.href='<?php echo G5_URL ?>/fighter/<?= $row['fighter_seq'] ?>';">
                            <h3><span class="weight">여성부</span> <span class="champ">1위</span></h3>
                            <div class="ranking_champ_name">
                                <span class="fighter_name"<? if($is_admin) {echo 'data-name-color="'.$row["fighter_status"].'"';} ?>><?= $row['fighter_name'] ?></span>
                                <div class="ranking_ring_name"><?= $row['fighter_ringname'] ?></div>
                                <div class="ranking_ring_name"><?= $row['team_name'] ?></div>
